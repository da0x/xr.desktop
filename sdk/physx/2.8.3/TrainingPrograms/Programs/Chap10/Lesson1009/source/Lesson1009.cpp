...
// Fluid globals
NxFluid* gFluid = NULL;
NxFluidEmitter* gFluidEmitter = NULL;
NxU32 gNumParticles = 0;
NxVec3* gParticles = 0;

#define REST_PARTICLES_PER_METER 10
#define KERNEL_RADIUS_MULTIPLIER 2.3
#define MOTION_LIMIT_MULTIPLIER 100
#define PACKET_SIZE_MULTIPLIER 64
...
void RenderActors(bool shadows)
{
    ...
    while (nbActors--)
    {
		...
		bool isDevision = false;
		for (NxActor** devision = gDevisions.begin(); devision != gDevisions.end(); devision++)
			if (actor == *devision)
				isDevision = true;
		if(isDevision) continue;
...
void RenderFluid()
{
	...
	glColor4f(1.f,1.0f,1.0f,5.0f);
	for (NxU32 p=0; p<gNumParticles; p++)
	{
		NxVec3& particle = gParticles[p];
		glPushMatrix();
		glTranslatef(particle.x,particle.y,particle.z);
		glutSolidCube(0.1); //glutSolidCube(0.1); //glutWireCube(0.05);  // glutSolidSphere(0.1,8,5);
		glPopMatrix();
	}
...
void InitGlut(int argc, char** argv, char* lessonTitle)
{
    ...
    // Setup lighting
    glEnable(GL_LIGHTING);
    float AmbientColor[]    = { 0.1f, 0.1f, 0.1f, 1.0f };         glLightfv(GL_LIGHT0, GL_AMBIENT, AmbientColor);
    float DiffuseColor[]    = { 0.6f, 0.6f, 0.6f, 1.0f };         glLightfv(GL_LIGHT0, GL_DIFFUSE, DiffuseColor);
    float SpecularColor[]   = { 0.4f, 0.4f, 0.4f, 1.0f };         glLightfv(GL_LIGHT0, GL_SPECULAR, SpecularColor);
...
NxFluid* CreateFluid()
{
	...
	memset(gParticles, 0, MAX_PARTICLES * sizeof(NxVec3));
	...
	if (!gHardwareCloth) // Simulate fluid in SW when no HW is available
...
void ReleaseFluid()
{
	if (gFluid)
	{
        if (gFluidEmitter)
        {
            gFluid->releaseEmitter(*gFluidEmitter);
            gFluidEmitter = 0;
        }

		if (gScene)  gScene->releaseFluid(*gFluid);
		gFluid = NULL;
	}
	if (gParticles)
	{
...
{
	PrintControls();
	InitGlut(argc, argv, "Lesson 1009: Interaction With Fluid");
    InitNx();
    glutMainLoop();
	ReleaseNx();
	return 0;
