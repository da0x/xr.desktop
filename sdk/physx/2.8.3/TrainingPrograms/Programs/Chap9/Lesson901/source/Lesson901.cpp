...
void InitializeHUD()
{
	...
	// Add hardware/software scene to HUD
	if (bHardwareScene)
...
void InitNx()
{
	...
	gPhysicsSDK->setParameter(NX_VISUALIZE_FLUID_POSITION, 0.5f);
	gPhysicsSDK->setParameter(NX_VISUALIZE_FLUID_PACKETS, 0.1f);
	...
	gFluid = CreateFluid(NxVec3(0,3,0), 20, 0.1, gScene);
...
{
	PrintControls();
	InitGlut(argc, argv, "Lesson 901: Fluid Creation");
    InitNx();
    glutMainLoop();
	ReleaseNx();
	return 0;
