...
void PrintControls()
{
    ...
    printf("\n Force Controls:\n ---------------\n i = +z, k = -z\n j = +x, l = -x\n u = +y, m = -y\n");
	printf("\n Miscellaneous:\n --------------\n p = Pause\n r = Select Next Actor\n f = Toggle Force Mode\n b = Toggle shape\n");
	printf("\n Special:\n --------\n v = Display Emitter Direction\n t = Release Fluid Emitter\n x = Toggle Repulsion on Actor\n");
}
...
void RenderCallback()
{
	...
	if (bEmitterWireframe&&fluidEmitter)
	{
...
/*		case 'x': { bShadows = !bShadows; break; }
		...
		case 'v': { 
			if(fluidEmitter)
			{
				bEmitterWireframe = !bEmitterWireframe;
			} 
			break; }
		case 'b': 
			{
				if(fluidEmitter)
				{
					NxShape* frameShape = fluidEmitter->getFrameShape();
					assert(frameShape);
					...
					{
						newActor = CreateCapsule(NxVec3(0,0,0), 0.2f, 0.9, 4);
					}
					else
					{
					...
					fluidEmitter->setFrameShape(newActor->getShapes()[0]);
				}
				else
				{
					printf("There isn't fluid emitter in the scene\n");
				}
			}
			break; 
		case 'x': 
			{
				if(fluidEmitter)
				{
					bool repulsion = fluidEmitter->getFlag(NX_FEF_FORCE_ON_BODY) > 0;
					fluidEmitter->setFlag(NX_FEF_FORCE_ON_BODY, !repulsion);
				}
			}
			break; 
		case 't': 
