...
{
	GetPhysicsResults();
	ReleaseUserDataFromActors(gScenes[0]);
	ReleaseUserDataFromActors(gScenes[1]);
...
void StartPhysics()
{
	...
	for (NxU32 i = 0; i < 2; ++i)
	{
	    if (0 != gScenes[i])
		{
	        // Start collision and dynamics for delta time since the last frame for both scenes
