...
//	NxBuildSmoothNormals(HEIGHTFIELD_NB_FACES, HEIGHTFIELD_NB_VERTS, gHeightfieldVerts, gHeightfieldFaces, NULL, gHeightfieldNormals, true);
	...
	//heightfieldShapeDesc.meshPagingMode              = NX_MESH_PAGING_AUTO;
    NxInitCooking();
    if (0)
    {
        // Cooking from file
	...
	// by API "meshData->getReferenceCount() == 0". And, the release API is "gPhysicsSDK->releaseTriangleMesh(*hfShapeDesc.meshData);"
	//
