...
void SampleExplosion::setup(NxScene* scene)
{
	assert(scene);
	m_scene = scene;
	...
	createExplosionForceField(NxVec3(0, 0, 0), m_kinematicActor, scene);
}
...
void SampleExplosion::update() 
{
	if (m_explosionFrame > 0)
	{
		...
		{
			m_forceField->addShapeGroup(*m_inclusionGroup);
			NxMat34 pose;
			pose.id();
		...
		if (m_explosionFrame > 100)
		{
			...
			m_forceField->removeShapeGroup(*m_inclusionGroup);
...
void SampleExplosion::cleanup()
{
	if (m_scene != NULL)
	{
		for (NxU32 i = 0; i < m_debrisActors.size(); i++)
		{
			...
			m_scene->releaseActor(*m_debrisActors[i]);
		}
