...
*/
	...
	bool SetParam( int which, int argc, const char **argv )
	{
		...
		for ( int i = 0; !found && (i < argc); ++i )
		{
			...
			else if ( !stricmp( buff, sNxParam_VisualizeBodyLinMomentum ) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_BODY_LIN_MOMENTUM, value );
#endif
			}
			else if ( !stricmp( buff, sNxParam_VisualizeBodyAngMomentum ) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_BODY_ANG_MOMENTUM, value );
#endif
			}
			else if ( !stricmp( buff, sNxParam_VisualizeBodyLinAccel ) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_BODY_LIN_ACCEL, value );
#endif
			}
			else if ( !stricmp( buff, sNxParam_VisualizeBodyAngAccel ) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_BODY_ANG_ACCEL, value );
#endif
			}
			else if ( !stricmp( buff, sNxParam_VisualizeBodyLinForce ) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_BODY_LIN_FORCE, value );
#endif
			}
			else if ( !stricmp( buff, sNxParam_VisualizeBodyAngForce ) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_BODY_ANG_FORCE, value );
#endif
			}
			else if ( !stricmp( buff, sNxParam_VisualizeBodyReduced	) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_BODY_REDUCED, value );
#endif
			}
			...
			else if ( !stricmp( buff, sNxParam_VisualizeBodyContactList ) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_BODY_CONTACT_LIST, value );
#endif
			}
			else if ( !stricmp( buff, sNxParam_VisualizeBodyJointList ) )
			{
				found = true;
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_BODY_JOINT_LIST, value );
#endif
			}
			else if ( !stricmp( buff, sNxParam_VisualizeBodyDamping ) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_BODY_DAMPING, value );
#endif
			}
			else if ( !stricmp( buff, sNxParam_VisualizeBodySleep ) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_BODY_SLEEP, value );
#endif
			}
			...
			else if ( !stricmp( buff, sNxParam_VisualizeJointError ) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_JOINT_ERROR, value );
#endif
			}
			else if ( !stricmp( buff, sNxParam_VisualizeJointForce ) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_JOINT_FORCE, value );
#endif
			}
			else if ( !stricmp( buff, sNxParam_VisualizeJointReduced ) )
			{
				found = true;				
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER < 270
				mContainer->SetParam( NX_VISUALIZE_JOINT_REDUCED, value );
#endif
			}
			...
			else if ( !stricmp( buff, sNxParam_MinSeparationForPenalty ) )
			{
				...
				float value = GetFirstFloat( i+1, argc, argv );
#if NX_SDK_VERSION_NUMBER >= 270
				mContainer->SetParam( NX_SKIN_WIDTH, value*-0.5f );
#else
