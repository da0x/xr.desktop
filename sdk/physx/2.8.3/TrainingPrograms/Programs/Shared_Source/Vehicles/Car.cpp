...
void createCarWithDesc(const NxVec3& pos, bool frontWheelDrive, bool backWheelDrive, bool corvette, bool monsterTruck, bool oldStyle, NxPhysicsSDK* physicsSDK)
	...
	vehicleDesc.motorForce				= monsterTruck?18000.f:3500.f;
	vehicleDesc.centerOfMass.set(0,0,0);
	vehicleDesc.maxVelocity				= (monsterTruck)?20.f:30.f;
	...
	for (NxU32 i = 0; i < 4; i++) {
		...
		wheelDesc[i].wheelRadius = (monsterTruck)?wheelRadius*2.f:wheelRadius;;
	...
	NxReal heightPos = (monsterTruck)?-1.f:-0.4f;
	wheelDesc[0].position.set( 1.8f, -0.4f, widthPos);
	wheelDesc[1].position.set( 1.8f, -0.4f,-widthPos);
	wheelDesc[2].position.set(-1.5f, -0.4f, widthPos);
	wheelDesc[3].position.set(-1.5f, -0.4f,-widthPos);
	...
	NxQuat q;
	q.setXYZW(0.0025808380f,0.35915127f,0.0011281641f,0.93327510f);
...
NxVehicle* createTruckPuller(const NxVec3& pos, NxU32 nbGears, bool oldStyle) {
	...
	return NxVehicle::createVehicle(gScene, truckPullerDesc);
}
...
NxVehicle* createFullTruck(const NxVec3& pos, NxU32 nbGears, bool has4Axes, bool oldStyle) {
	...
	NxVehicle* truck = NxVehicle::createVehicle(gScene, fullTruck);

	NxAllVehicles::AddVehicle(truck);
...
void renderHUD(OrthographicDrawing& orthoDraw) {
	...
	glColor4f(GL_COLOR_LIGHT_BLUE);
	orthoDraw.drawLine(centerX, centerY, centerX + NxMath::cos(angle)*radius*0.9f, centerY - NxMath::sin(angle)*radius*0.9f);
	{
		char buf[100]; sprintf(buf, "km/h: %3d", (int)kmh);
		orthoDraw.drawText(centerX-35, centerY+radius-10.f, 1, 1, buf);
	}
	//printf("%2.3f %2.3f (%2.3f%%)\n", velocity, maxV, velocity01*100.f);

	// Rpm
	centerY = height-30.f-3*radius;
	if (curVehicle->getMotor()) {
		...
		glColor4f(GL_COLOR_WHITE);
		orthoDraw.drawCircle(centerX, centerY, radius, -30, 210, 10, 5, 10);
		glColor4f(GL_COLOR_LIGHT_BLUE);
		orthoDraw.drawLine(centerX, centerY, centerX + NxMath::cos(angle)*radius*0.9f, centerY - NxMath::sin(angle)*radius*0.9f);
	...
	if (curVehicle->getGears()) {
		...
		orthoDraw.drawText(centerX, centerY-radius-10.f, 1, 1, buf);
	}
	...
	//orthoDraw.drawText(10,10,3,"a\nb\nc\nd");
	//orthoDraw.drawText(NxMath::rand(10.f,400.f),NxMath::rand(50.f,500.f),3,"helo\n");
