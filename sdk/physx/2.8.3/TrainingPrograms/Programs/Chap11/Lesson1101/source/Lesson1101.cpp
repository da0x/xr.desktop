...
void InitGlut(int argc, char** argv, char* lessonTitle)
{
	...
	float Position[]        = { 100.0f, 100.0f, -0, 1.0f };  glLightfv(GL_LIGHT0, GL_POSITION, Position);
	glEnable(GL_LIGHT0);
} 
//------------------------------------------------------------------------------------
// create soft body cube: 125 vertcie and 320 tetrahedron.
void SetupSoftBodyCube()
{
	...
	/*int a = softBody->getNxSoftBody()->getNumberOfParticles();
	NxSoftBodyMeshDesc dd;
	softBody->getNxSoftBody()->getSoftBodyMesh()->saveToDesc(dd);
	a = dd.numTetrahedra;*/
	//CreateSphere(NxVec3(0,13,0),1,0);

	// set camera position and direction
	gCameraPos.set(0.0f, 9.0f, 14.0f);
...
{
	PrintControls();
	InitGlut(argc, argv, "Lesson 1101: SoftBodyCube");
	InitNx();
	glutMainLoop();
	ReleaseNx();
	return 0;
