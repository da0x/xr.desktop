...
//
// ===============================================================================
// simulation soft body with self collision.
//
// 
#include <GL/glut.h>
#include <stdio.h>

#include "Lesson1104.h"
#include "Timing.h"
	...
	*/
	...
	// Render all the SoftBodys in the scene
	{
		...
		for (MySoftBody **softBody = gSoftBodies.begin(); softBody != gSoftBodies.end(); softBody++){
			...
			(*softBody)->simulateAndDraw(shadows, true, false);
		}
	}
...
void InitGlut(int argc, char** argv, char* lessonTitle)
{
	...
	float Position[]        = { 10.0f, 100.0f, -0, 1.0f };  glLightfv(GL_LIGHT0, GL_POSITION, Position);
	glEnable(GL_LIGHT0);
}
...
void SetupSelfCollisionScene()
{
	...
	MySoftBody *softBody = new MySoftBody(gScene, softBodyDesc, FindMediaFile(tetFileName,s), objMesh,0.2);
	...
	softBody = new MySoftBody(gScene, softBodyDesc, FindMediaFile(tetFileName,s), objMesh,0.2);
	if (!softBody->getNxSoftBody())
	{
		printf("Error: Unable to create the SoftBody for the current scene.\n");
		delete softBody;
...
{
	PrintControls();
	InitGlut(argc, argv, "Lesson 1104: Self-Collision");
//	SetDataDirectory();
	InitNx();
	glutMainLoop();
	ReleaseNx();
...
void StartPhysics()
{
	...
	gScene->flushStream();
	//gScene->fetchResults(NX_RIGID_BODY_FINISHED, true, NULL);
}
