...
void SetupTearingScene()
{
	...
	NxMat33 rot;
	rot.rotX(NxPiF32);
	softBodyDesc.globalPose.t = NxVec3(4.0f, 5, 3.4f);
	softBodyDesc.globalPose.M = rot;
	softBodyDesc.stretchingStiffness = 1.0;
	MySoftBody* softBody = new MySoftBody(gScene, softBodyDesc,SOFTBODY_BOX,3,3,3,1);
	...
	gCameraPos.set(-7.0f, 10.0f, 10.0f);
...
{
	PrintControls();
	InitGlut(argc, argv, "Lesson 1106: Tearing");
	//	SetDataDirectory();
	InitNx();
	glutMainLoop();
	ReleaseNx();
