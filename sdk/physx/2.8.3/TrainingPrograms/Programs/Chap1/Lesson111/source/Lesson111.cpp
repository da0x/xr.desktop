	...
	case ' ': 
		{
			...
			NxActor* sphere = CreateSphere(gCameraPos, 0.3, 10);
			sphere->setLinearVelocity(gCameraForward * 50);
			break; 
		}
...
void SpecialKeys(int key, int x, int y)
{
	switch (key)
	{
		...
		{ 
			...
			hud.SetDisplayString(1, ds, 0.015f, 0.92f);
			gPhysicsSDK->setParameter(NX_CONTINUOUS_CD, bCCDEnabled);
			break; 
		}
		case ' ': { bFireCCDBox = true; break; }	
	}
}
...
void InitGlut(int argc, char** argv)
{
    ...
    gMainHandle = glutCreateWindow("Lesson 111: Dynamic CCD");
...
void CreateStack(int size)
{
	...
	while(size)
	{
		for(int i=0;i<size;i++)
		{
			...
			gSelectedActor = CreateCCDBox(pos, NxVec3(cubeSize, cubeSize, cubeSize), 10, false);
		}
